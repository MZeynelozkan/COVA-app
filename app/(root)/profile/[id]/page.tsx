...
const Page = async ({ params, searchParams }: URLProps) => {
  ...
  return (
    <>
      {/* Add metadata using Head component */}
      <Head>
        <title>{user?.name} | User Profile</title>
        <meta name="description" content={`${user?.name}'s profile on the platform. View their collections and activity.`} />
        <meta name="author" content={user?.name} />
        <meta property="og:title" content={`${user?.name} | User Profile`} />
        <meta property="og:description" content={`${user?.name}'s profile on the platform. View their collections and activity.`} />
        <meta property="og:image" content={user?.image ?? "https://cdn.usegalileo.ai/sdxl10/f496ae30-5f5e-4a7c-a750-7cf9e72e4417.png"} />
        <meta property="og:url" content={`https://your-website.com/profile/${id}`} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${user?.name} | User Profile`} />
        <meta name="twitter:description" content={`${user?.name}'s profile on the platform. View their collections and activity.`} />
        <meta name="twitter:image" content={user?.image ?? "https://cdn.usegalileo.ai/sdxl10/f496ae30-5f5e-4a7c-a750-7cf9e72e4417.png"} />
      </Head>
